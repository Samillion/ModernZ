...
local function set_osc_styles()
    ...
    osc_styles = {
        ...
        element_down = "{\\1c&H" .. osc_color_convert(user_opts.held_element_color) .. "&" .. (string.format("\\fscx%s\\fscy%s", user_opts.button_held_size, user_opts.button_held_size) or "") .. "}",
    ...
    local function render_element(n)
        ...
        if element.type == "button" and hover_effects.box
            ...
            if mouse_hit(element) and is_clickable and element.enabled then
                ...
                if state.mouse_down_counter > 0 then
                    hover_base_alpha = user_opts.button_held_box_alpha
                end
